...
short_description: List Snowflake database roles
description:
  - Retrieve information about database roles in Snowflake.
  - This module is read-only and does not modify any resources.
version_added: "1.0.0"
author: Steve Fulmer (@stevefulme1)
options:
  name:
    description: Filter to a specific database role name or pattern.
    type: str
  limit:
    description:
...
EXAMPLES = r"""
- name: List all database roles
  stevefulme1.snowflake.database_role_info:
    account: myaccount
    user: myuser
    private_key: "{{ private_key }}"

- name: Get a specific database role
  stevefulme1.snowflake.database_role_info:
    account: myaccount
    user: myuser
    private_key: "{{ private_key }}"
    name: my_database role
"""
...
def run_module():
    ...
    try:
        ...
        sql = "SHOW DATABASE ROLES IN DATABASE {name}"
    ...
    module.exit_json(changed=False, database_roles=rows)
