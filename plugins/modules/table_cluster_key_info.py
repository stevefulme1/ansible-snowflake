...
DOCUMENTATION = r"""
---
module: table_cluster_key_info
short_description: List Snowflake table cluster keys
description:
  - Retrieve information about table cluster keys in Snowflake.
  - This module is read-only and does not modify any resources.
version_added: "1.0.0"
author: Steve Fulmer (@stevefulme1)
options:
  name:
    description: Filter to a specific table cluster key name or pattern.
    type: str
  limit:
    description:
...
EXAMPLES = r"""
- name: List all table cluster keys
  stevefulme1.snowflake.table_cluster_key_info:
    account: myaccount
    user: myuser
    private_key: "{{ private_key }}"

- name: Get a specific table cluster key
  stevefulme1.snowflake.table_cluster_key_info:
    account: myaccount
    user: myuser
    private_key: "{{ private_key }}"
    name: my_table cluster key
"""

RETURN = r"""
table_cluster_keys:
  description: List of table cluster key records.
  type: list
  returned: always
  elements: dict
"""
...
def run_module():
    ...
    module.exit_json(changed=False, table_cluster_keys=rows)
