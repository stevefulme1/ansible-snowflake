...
DOCUMENTATION = r"""
---
module: warehouse_resize_info
short_description: List Snowflake warehouse size configurations
description:
  - Retrieve information about warehouse size configurations in Snowflake.
  - This module is read-only and does not modify any resources.
version_added: "1.0.0"
author: Steve Fulmer (@stevefulme1)
options:
  name:
    description: Filter to a specific warehouse resize name or pattern.
    type: str
  limit:
    description:
...
EXAMPLES = r"""
- name: List all warehouse size configurations
  stevefulme1.snowflake.warehouse_resize_info:
    account: myaccount
    user: myuser
    private_key: "{{ private_key }}"

- name: Get a specific warehouse resize
  stevefulme1.snowflake.warehouse_resize_info:
    account: myaccount
    user: myuser
    private_key: "{{ private_key }}"
    name: my_warehouse resize
"""

RETURN = r"""
warehouse_sizes:
  description: List of warehouse resize records.
  type: list
  returned: always
  elements: dict
"""
...
def run_module():
    ...
    module.exit_json(changed=False, warehouse_sizes=rows)
