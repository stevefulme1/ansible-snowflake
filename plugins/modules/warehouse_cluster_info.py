...
DOCUMENTATION = r"""
---
module: warehouse_cluster_info
short_description: List Snowflake warehouse clusters
description:
  - Retrieve information about warehouse clusters in Snowflake.
  - This module is read-only and does not modify any resources.
version_added: "1.0.0"
author: Steve Fulmer (@stevefulme1)
options:
  name:
    description: Filter to a specific warehouse cluster name or pattern.
    type: str
  limit:
    description:
...
EXAMPLES = r"""
- name: List all warehouse clusters
  stevefulme1.snowflake.warehouse_cluster_info:
    account: myaccount
    user: myuser
    private_key: "{{ private_key }}"

- name: Get a specific warehouse cluster
  stevefulme1.snowflake.warehouse_cluster_info:
    account: myaccount
    user: myuser
    private_key: "{{ private_key }}"
    name: my_warehouse cluster
"""

RETURN = r"""
warehouse_clusters:
  description: List of warehouse cluster records.
  type: list
  returned: always
  elements: dict
"""
...
def run_module():
    ...
    module.exit_json(changed=False, warehouse_clusters=rows)
