...
short_description: Manage Snowflake replication groups
description:
  - Create or alter a replication group.
version_added: "1.1.0"
...
options:
  name:
    ...
    required: true
  object_types:
    description: Object types to replicate.
    type: list
    elements: str
  allowed_databases:
    description: Databases to replicate.
    type: list
    elements: str
  allowed_accounts:
...
- name: Create replication group
  stevefulme1.snowflake.replication_group:
    name: MY_RG
...
def run_module():
    ...
    if state == "absent":
        sql = "DROP REPLICATION GROUP IF EXISTS {0}".format(name)
    else:
