...
def run_module():
    argument_spec = dict(
        ...
        frequency=dict(type='str', default='MONTHLY',
                        choices=['MONTHLY', 'DAILY', 'WEEKLY', 'YEARLY', 'NEVER']),
    ...
    if state == 'absent':
        ...
        parts = ['CREATE OR REPLACE RESOURCE MONITOR {0}'.format(
            SnowflakeClient.quote_identifier(name))]
        ...
        if module.params.get('start_timestamp'):
            parts.append("START_TIMESTAMP = '{0}'".format(module.params['start_timestamp']))
        else:
        ...
        if module.params.get('suspend_immediately_at'):
            triggers.append('ON {0} PERCENT DO SUSPEND_IMMEDIATE'.format(
