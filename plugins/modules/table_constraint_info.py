...
DOCUMENTATION = r"""
---
module: table_constraint_info
short_description: List Snowflake table constraints
description:
  - Retrieve information about table constraints in Snowflake.
  - This module is read-only and does not modify any resources.
version_added: "1.0.0"
author: Steve Fulmer (@stevefulme1)
options:
  name:
    description: Filter to a specific table constraint name or pattern.
    type: str
  limit:
    description:
...
EXAMPLES = r"""
- name: List all table constraints
  stevefulme1.snowflake.table_constraint_info:
    account: myaccount
    user: myuser
    private_key: "{{ private_key }}"

- name: Get a specific table constraint
  stevefulme1.snowflake.table_constraint_info:
    account: myaccount
    user: myuser
    private_key: "{{ private_key }}"
    name: my_table constraint
"""

RETURN = r"""
constraints:
  description: List of table constraint records.
  type: list
  returned: always
  elements: dict
"""
...
def run_module():
    ...
    module.exit_json(changed=False, constraints=rows)
