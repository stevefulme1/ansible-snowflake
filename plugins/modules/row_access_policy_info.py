...
DOCUMENTATION = r"""
---
module: row_access_policy_info
short_description: List Snowflake row access policies
description:
  - Retrieve information about row access policies in Snowflake.
  - This module is read-only and does not modify any resources.
version_added: "1.0.0"
author: Steve Fulmer (@stevefulme1)
options:
  name:
    description: Filter to a specific row access policy name or pattern.
    type: str
  limit:
    description:
...
EXAMPLES = r"""
- name: List all row access policies
  stevefulme1.snowflake.row_access_policy_info:
    account: myaccount
    user: myuser
    private_key: "{{ private_key }}"

- name: Get a specific row access policy
  stevefulme1.snowflake.row_access_policy_info:
    account: myaccount
    user: myuser
    private_key: "{{ private_key }}"
    name: my_row access policy
"""

RETURN = r"""
row_access_policies:
  description: List of row access policy records.
  type: list
  returned: always
  elements: dict
"""
...
def run_module():
    ...
    module.exit_json(changed=False, row_access_policies=rows)
