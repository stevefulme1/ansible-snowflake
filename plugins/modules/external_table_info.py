...
DOCUMENTATION = r"""
---
module: external_table_info
short_description: List Snowflake external tables
description:
  - Retrieve information about external tables in Snowflake.
  - This module is read-only and does not modify any resources.
version_added: "1.0.0"
author: Steve Fulmer (@stevefulme1)
options:
  name:
    description: Filter to a specific external table name or pattern.
    type: str
  limit:
    description:
...
EXAMPLES = r"""
- name: List all external tables
  stevefulme1.snowflake.external_table_info:
    account: myaccount
    user: myuser
    private_key: "{{ private_key }}"

- name: Get a specific external table
  stevefulme1.snowflake.external_table_info:
    account: myaccount
    user: myuser
    private_key: "{{ private_key }}"
    name: my_external table
"""

RETURN = r"""
external_tables:
  description: List of external table records.
  type: list
  returned: always
  elements: dict
"""
...
def run_module():
    ...
    module.exit_json(changed=False, external_tables=rows)
