...
DOCUMENTATION = r"""
---
module: tag_association_info
short_description: List Snowflake tag associations
description:
  - Retrieve information about tag associations in Snowflake.
  - This module is read-only and does not modify any resources.
version_added: "1.0.0"
author: Steve Fulmer (@stevefulme1)
options:
  name:
    description: Filter to a specific tag association name or pattern.
    type: str
  limit:
    description:
...
EXAMPLES = r"""
- name: List all tag associations
  stevefulme1.snowflake.tag_association_info:
    account: myaccount
    user: myuser
    private_key: "{{ private_key }}"

- name: Get a specific tag association
  stevefulme1.snowflake.tag_association_info:
    account: myaccount
    user: myuser
    private_key: "{{ private_key }}"
    name: my_tag association
"""

RETURN = r"""
tag_associations:
  description: List of tag association records.
  type: list
  returned: always
  elements: dict
"""
...
def run_module():
    ...
    module.exit_json(changed=False, tag_associations=rows)
