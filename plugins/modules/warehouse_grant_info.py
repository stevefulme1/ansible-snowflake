...
DOCUMENTATION = r"""
---
module: warehouse_grant_info
short_description: List Snowflake warehouse grants
description:
  - Retrieve information about warehouse grants in Snowflake.
  - This module is read-only and does not modify any resources.
version_added: "1.0.0"
author: Steve Fulmer (@stevefulme1)
options:
  name:
    description: Filter to a specific warehouse grant name or pattern.
    type: str
  limit:
    description:
...
EXAMPLES = r"""
- name: List all warehouse grants
  stevefulme1.snowflake.warehouse_grant_info:
    account: myaccount
    user: myuser
    private_key: "{{ private_key }}"

- name: Get a specific warehouse grant
  stevefulme1.snowflake.warehouse_grant_info:
    account: myaccount
    user: myuser
    private_key: "{{ private_key }}"
    name: my_warehouse grant
"""

RETURN = r"""
warehouse_grants:
  description: List of warehouse grant records.
  type: list
  returned: always
  elements: dict
"""
...
def run_module():
    ...
    module.exit_json(changed=False, warehouse_grants=rows)
