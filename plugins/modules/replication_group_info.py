...
short_description: List Snowflake replication groups
description:
  - Retrieve information about replication groups in Snowflake.
  - This module is read-only and does not modify any resources.
version_added: "1.0.0"
author: Steve Fulmer (@stevefulme1)
options:
  name:
    description: Filter to a specific replication group name or pattern.
    type: str
  limit:
    description:
...
- name: List all replication groups
  stevefulme1.snowflake.replication_group_info:
    account: myaccount
    user: myuser
    private_key: "{{ private_key }}"

- name: Get a specific replication group
  stevefulme1.snowflake.replication_group_info:
    account: myaccount
    user: myuser
    private_key: "{{ private_key }}"
    name: my_replication group
"""

RETURN = r"""
replication_groups:
  description: List of replication group records.
  type: list
  returned: always
  elements: dict
"""
...
def run_module():
    ...
    module.exit_json(changed=False, replication_groups=rows)
