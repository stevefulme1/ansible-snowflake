...
DOCUMENTATION = r"""
---
module: session_policy_info
short_description: List Snowflake session policies
description:
  - Retrieve information about session policies in Snowflake.
  - This module is read-only and does not modify any resources.
version_added: "1.0.0"
author: Steve Fulmer (@stevefulme1)
options:
  name:
    description: Filter to a specific session policy name or pattern.
    type: str
  limit:
    description:
...
EXAMPLES = r"""
- name: List all session policies
  stevefulme1.snowflake.session_policy_info:
    account: myaccount
    user: myuser
    private_key: "{{ private_key }}"

- name: Get a specific session policy
  stevefulme1.snowflake.session_policy_info:
    account: myaccount
    user: myuser
    private_key: "{{ private_key }}"
    name: my_session policy
"""

RETURN = r"""
session_policies:
  description: List of session policy records.
  type: list
  returned: always
  elements: dict
"""
...
def run_module():
    ...
    module.exit_json(changed=False, session_policies=rows)
