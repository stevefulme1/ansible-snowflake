...
DOCUMENTATION = r"""
---
module: user_grant_info
short_description: List Snowflake user grants
description:
  - Retrieve information about user grants in Snowflake.
  - This module is read-only and does not modify any resources.
version_added: "1.0.0"
author: Steve Fulmer (@stevefulme1)
options:
  name:
    description: Filter to a specific user grant name or pattern.
    type: str
  limit:
    description:
...
EXAMPLES = r"""
- name: List all user grants
  stevefulme1.snowflake.user_grant_info:
    account: myaccount
    user: myuser
    private_key: "{{ private_key }}"

- name: Get a specific user grant
  stevefulme1.snowflake.user_grant_info:
    account: myaccount
    user: myuser
    private_key: "{{ private_key }}"
...
RETURN = r"""
user_grants:
  description: List of user grant records.
  type: list
  returned: always
  elements: dict
"""
...
def run_module():
    ...
    module.exit_json(changed=False, user_grants=rows)
