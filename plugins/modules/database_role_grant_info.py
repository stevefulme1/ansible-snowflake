...
DOCUMENTATION = r"""
---
module: database_role_grant_info
short_description: List Snowflake database role grants
description:
  - Retrieve information about database role grants in Snowflake.
  - This module is read-only and does not modify any resources.
version_added: "1.0.0"
author: Steve Fulmer (@stevefulme1)
options:
  name:
    description: Filter to a specific database role grant name or pattern.
    type: str
  limit:
    description:
...
EXAMPLES = r"""
- name: List all database role grants
  stevefulme1.snowflake.database_role_grant_info:
    account: myaccount
    user: myuser
    private_key: "{{ private_key }}"

- name: Get a specific database role grant
  stevefulme1.snowflake.database_role_grant_info:
    account: myaccount
    user: myuser
    private_key: "{{ private_key }}"
    name: my_database role grant
"""

RETURN = r"""
database_role_grants:
  description: List of database role grant records.
  type: list
  returned: always
  elements: dict
"""
...
def run_module():
    ...
    try:
        ...
        sql = "SHOW GRANTS TO DATABASE ROLE"
    ...
    module.exit_json(changed=False, database_role_grants=rows)
