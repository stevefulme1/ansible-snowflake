...
DOCUMENTATION = r"""
---
module: result_cache_info
short_description: List Snowflake result cache settings
description:
  - Retrieve information about result cache settings in Snowflake.
  - This module is read-only and does not modify any resources.
version_added: "1.0.0"
author: Steve Fulmer (@stevefulme1)
options:
  name:
    description: Filter to a specific result cache name or pattern.
    type: str
  limit:
    description:
...
EXAMPLES = r"""
- name: List all result cache settings
  stevefulme1.snowflake.result_cache_info:
    account: myaccount
    user: myuser
    private_key: "{{ private_key }}"

- name: Get a specific result cache
  stevefulme1.snowflake.result_cache_info:
    account: myaccount
    user: myuser
    private_key: "{{ private_key }}"
    name: my_result cache
"""

RETURN = r"""
result_cache_settings:
  description: List of result cache records.
  type: list
  returned: always
  elements: dict
"""
...
def run_module():
    ...
    module.exit_json(changed=False, result_cache_settings=rows)
