...
def schema_exists(client, db, name):
    sql = "SHOW SCHEMAS LIKE '{0}'".format(name)
    if db:
...
def run_module():
    ...
    try:
        ...
        fqn = '{0}.{1}'.format(client.quote_identifier(db), client.quote_identifier(name)) if db else client.quote_identifier(name)
        ...
        if state == 'absent':
            ...
            if not exists:
                ...
                if module.params.get('data_retention_time_in_days') is not None:
                    parts.append('DATA_RETENTION_TIME_IN_DAYS = {0}'.format(
                ...
                alterations = []
                if module.params.get('data_retention_time_in_days') is not None:
                    alterations.append('DATA_RETENTION_TIME_IN_DAYS = {0}'.format(
                        module.params['data_retention_time_in_days']))
                if module.params.get('comment'):
