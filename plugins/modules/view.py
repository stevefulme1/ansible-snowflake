...
---
module: view
short_description: Manage Snowflake views
description:
  - Create, alter, or drop a Snowflake view.
version_added: "1.1.0"
...
options:
  ...
  query:
    ...
    type: str
  secure:
    description: Create as a secure view.
    type: bool
    default: false
  comment:
...
def run_module():
    ...
    if state == "absent":
        sql = "DROP VIEW IF EXISTS {0}".format(fqn)
    else:
        q = module.params.get("query")
        if not q:
            module.fail_json(msg="query is required when state=present")
        prefix = "CREATE OR REPLACE SECURE VIEW" if module.params["secure"] else "CREATE OR REPLACE VIEW"
