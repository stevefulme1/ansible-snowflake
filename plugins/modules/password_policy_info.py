...
DOCUMENTATION = r"""
---
module: password_policy_info
short_description: List Snowflake password policies
description:
  - Retrieve information about password policies in Snowflake.
  - This module is read-only and does not modify any resources.
version_added: "1.0.0"
author: Steve Fulmer (@stevefulme1)
options:
  name:
    description: Filter to a specific password policy name or pattern.
    type: str
  limit:
    description:
...
EXAMPLES = r"""
- name: List all password policies
  stevefulme1.snowflake.password_policy_info:
    account: myaccount
    user: myuser
    private_key: "{{ private_key }}"

- name: Get a specific password policy
  stevefulme1.snowflake.password_policy_info:
    account: myaccount
    user: myuser
    private_key: "{{ private_key }}"
    name: my_password policy
"""

RETURN = r"""
password_policies:
  description: List of password policy records.
  type: list
  returned: always
  elements: dict
"""
...
def run_module():
    ...
    module.exit_json(changed=False, password_policies=rows)
