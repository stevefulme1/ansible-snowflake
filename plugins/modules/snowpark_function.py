...
DOCUMENTATION = r"""
---
module: snowpark_function
short_description: Manage Snowflake user-defined functions
description:
  - Create or drop a UDF (Python, Java, Scala, SQL).
version_added: "1.1.0"
...
options:
  ...
  packages:
    ...
    elements: str
  body:
    description: Inline function body.
    type: str
  state:
...
- name: Create Python UDF
  stevefulme1.snowflake.snowpark_function:
    name: DOUBLE_IT
    schema_name: PUBLIC
...
def run_module():
    ...
    if state == "absent":
        sql = "DROP FUNCTION IF EXISTS {0}({1})".format(fqn, args)
    else:
        lang = module.params["language"].upper()
        parts = [
            "CREATE OR REPLACE FUNCTION {0}({1})".format(fqn, args),
