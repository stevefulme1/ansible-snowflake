...
class TestPipeCreate:
    ...
    def test_missing_copy_fails(self, mc, cc):
        m = MagicMock(); m.check_mode = False; m.params = dict(BASE, copy_statement=None); mc.return_value = m
        c = MagicMock(); c.query.return_value = []; cc.return_value = c
        pipe.run_module()
...
class TestPipeDelete:
    ...
    def test_drop(self, mc, cc):
        ...
        c = MagicMock(); c.query.return_value = [{"name": "RAW_PIPE"}]; cc.return_value = c
        pipe.run_module()
