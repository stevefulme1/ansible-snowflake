...
from ansible_collections.stevefulme1.snowflake.plugins.modules import cortex_search_service
...
class TestDocumentation:
    ...
    def test_documentation_exists(self):
        assert hasattr(cortex_search_service, "DOCUMENTATION")
    ...
    def test_documentation_has_options(self):
        assert "options" in cortex_search_service.DOCUMENTATION

    def test_examples_exist(self):
        assert hasattr(cortex_search_service, "EXAMPLES")
        assert len(cortex_search_service.EXAMPLES) > 0

    def test_examples_contain_fqcn(self):
        assert "stevefulme1.snowflake" in cortex_search_service.EXAMPLES

    def test_return_exists(self):
        assert hasattr(cortex_search_service, "RETURN")
        assert len(cortex_search_service.RETURN) > 0
...
class TestCreate:
    ...
    def test_create_executes_sql(self, mock_cursor):
        ...
        mock_cursor.execute("SHOW CORTEX_SEARCH_SERVICES LIKE 'test'")
        mock_cursor.execute.assert_called()

    def test_create_new_resource(self, mock_cursor):
        mock_cursor.fetchone.return_value = None
        result = {"changed": True, "cortex_search_service": {"name": "test"}}
        assert result["changed"] is True

    def test_create_existing_no_change(self, mock_cursor):
        mock_cursor.fetchone.return_value = ("test",)
        result = {"changed": False, "cortex_search_service": {"name": "test"}}
...
class TestDelete:
    ...
    def test_delete_existing(self, mock_cursor):
        mock_cursor.fetchone.return_value = ("test",)
        mock_cursor.execute("DROP CORTEX_SEARCH_SERVICE test")
        result = {"changed": True}
        assert result["changed"] is True
...
class TestErrorHandling:
    ...
    def test_connection_error(self, mock_cursor):
        ...
        with pytest.raises(Exception, match="Connection refused"):
            mock_cursor.execute("SHOW CORTEX_SEARCH_SERVICES")
    ...
    def test_permission_denied(self, mock_cursor):
        ...
        with pytest.raises(Exception, match="Insufficient privileges"):
            mock_cursor.execute("CREATE CORTEX_SEARCH_SERVICE test")
