...
class TestViewCreate:
    ...
    def test_create_secure(self, mc, cc):
        m = MagicMock(); m.check_mode = False; m.params = dict(BASE, secure=True); mc.return_value = m
        c = MagicMock(); cc.return_value = c
        view.run_module()
    ...
    def test_missing_query_fails(self, mc, cc):
        m = MagicMock(); m.check_mode = False; m.params = dict(BASE, query=None); mc.return_value = m
        c = MagicMock(); cc.return_value = c
        view.run_module()
