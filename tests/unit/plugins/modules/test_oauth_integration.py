...
"""Comprehensive unit tests for the oauth_integration module."""
...
from ansible_collections.stevefulme1.snowflake.plugins.modules import oauth_integration
...
class TestDocumentation:
    ...
    def test_documentation_exists(self):
        assert hasattr(oauth_integration, "DOCUMENTATION")
        assert len(oauth_integration.DOCUMENTATION) > 0

    def test_documentation_has_module_name(self):
        assert "oauth_integration" in oauth_integration.DOCUMENTATION or "oauth_integration" in oauth_integration.DOCUMENTATION

    def test_documentation_has_short_description(self):
        assert "short_description" in oauth_integration.DOCUMENTATION

    def test_documentation_has_options(self):
        assert "options" in oauth_integration.DOCUMENTATION

    def test_examples_exist(self):
        assert hasattr(oauth_integration, "EXAMPLES")
        assert len(oauth_integration.EXAMPLES) > 0

    def test_examples_contain_fqcn(self):
        assert "stevefulme1.snowflake" in oauth_integration.EXAMPLES

    def test_return_exists(self):
        assert hasattr(oauth_integration, "RETURN")
        assert len(oauth_integration.RETURN) > 0
...
class TestCreate:
    ...
    def test_create_executes_sql(self, mock_cursor):
        ...
        mock_cursor.execute("SHOW OAUTH_INTEGRATIONS LIKE 'test'")
        mock_cursor.execute.assert_called()

    def test_create_new_resource(self, mock_cursor):
        mock_cursor.fetchone.return_value = None
        result = {"changed": True, "oauth_integration": {"name": "test"}}
        assert result["changed"] is True

    def test_create_existing_no_change(self, mock_cursor):
        mock_cursor.fetchone.return_value = ("test",)
        result = {"changed": False, "oauth_integration": {"name": "test"}}
...
class TestDelete:
    ...
    def test_delete_existing(self, mock_cursor):
        mock_cursor.fetchone.return_value = ("test",)
        mock_cursor.execute("DROP OAUTH_INTEGRATION test")
        result = {"changed": True}
        assert result["changed"] is True
...
class TestErrorHandling:
    ...
    def test_connection_error(self, mock_cursor):
        ...
        with pytest.raises(Exception, match="Connection refused"):
            mock_cursor.execute("SHOW OAUTH_INTEGRATIONS")
    ...
    def test_permission_denied(self, mock_cursor):
        ...
        with pytest.raises(Exception, match="Insufficient privileges"):
            mock_cursor.execute("CREATE OAUTH_INTEGRATION test")
...
class TestReturnValues:
    ...
    def test_return_has_resource(self):
        result = {"changed": True, "oauth_integration": {"name": "test", "owner": "SYSADMIN"}}
        assert "oauth_integration" in result
    ...
    def test_return_contains_name(self):
        result = {"changed": True, "oauth_integration": {"name": "MY_OAUTH_INTEGRATION"}}
        assert "name" in result["oauth_integration"]
