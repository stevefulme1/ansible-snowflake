...
class TestStageCreate:
    ...
    def test_create_external(self, mc, cc):
        m = MagicMock(); m.check_mode = False
        m.params = dict(BASE, stage_type="external", url="s3://bucket/path/", storage_integration="S3_INT")
        mc.return_value = m; c = MagicMock(); cc.return_value = c
        stage.run_module()
