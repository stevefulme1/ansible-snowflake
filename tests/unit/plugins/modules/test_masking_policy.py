...
class TestDocumentation:
    ...
    def test_documentation_exists(self):
        assert hasattr(masking_policy, "DOCUMENTATION")
        assert len(masking_policy.DOCUMENTATION) > 0

    def test_documentation_has_module_name(self):
        assert "masking_policy" in masking_policy.DOCUMENTATION or "masking_policy" in masking_policy.DOCUMENTATION

    def test_documentation_has_short_description(self):
        assert "short_description" in masking_policy.DOCUMENTATION

    def test_documentation_has_options(self):
        assert "options" in masking_policy.DOCUMENTATION

    def test_examples_exist(self):
        assert hasattr(masking_policy, "EXAMPLES")
        assert len(masking_policy.EXAMPLES) > 0

    def test_examples_contain_fqcn(self):
        assert "stevefulme1.snowflake" in masking_policy.EXAMPLES

    def test_return_exists(self):
        assert hasattr(masking_policy, "RETURN")
        assert len(masking_policy.RETURN) > 0
...
class TestDelete:
    ...
    def test_delete_existing(self, mock_cursor):
        mock_cursor.fetchone.return_value = ("test",)
        mock_cursor.execute("DROP MASKING_POLICY test")
        result = {"changed": True}
        assert result["changed"] is True
...
class TestErrorHandling:
    ...
    def test_connection_error(self, mock_cursor):
        ...
        with pytest.raises(Exception, match="Connection refused"):
            mock_cursor.execute("SHOW MASKING_POLICYS")
    ...
    def test_permission_denied(self, mock_cursor):
        ...
        with pytest.raises(Exception, match="Insufficient privileges"):
            mock_cursor.execute("CREATE MASKING_POLICY test")
