...
class TestStreamCreate:
    ...
    def test_create_append_only(self, mc, cc):
        m = MagicMock(); m.check_mode = False; m.params = dict(BASE, append_only=True); mc.return_value = m
        c = MagicMock(); cc.return_value = c
        stream.run_module()
    ...
    def test_missing_source_fails(self, mc, cc):
        m = MagicMock(); m.check_mode = False; m.params = dict(BASE, source_table=None); mc.return_value = m
        c = MagicMock(); cc.return_value = c
        stream.run_module()
