...
class TestTaskCreate:
    ...
    def test_missing_sql_fails(self, mc, cc):
        m = MagicMock(); m.check_mode = False; m.params = dict(BASE, sql_statement=None); mc.return_value = m
        c = MagicMock(); c.query.return_value = []; cc.return_value = c
        task.run_module()
...
class TestTaskDelete:
    ...
    def test_drop(self, mc, cc):
        ...
        c = MagicMock(); c.query.return_value = [{"name": "HOURLY_LOAD"}]; cc.return_value = c
        task.run_module()
