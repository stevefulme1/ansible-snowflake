...
from ansible_collections.stevefulme1.snowflake.plugins.modules import row_access_policy
...
class TestDocumentation:
    ...
    def test_documentation_exists(self):
        assert hasattr(row_access_policy, "DOCUMENTATION")
    ...
    def test_documentation_has_options(self):
        assert "options" in row_access_policy.DOCUMENTATION

    def test_examples_exist(self):
        assert hasattr(row_access_policy, "EXAMPLES")
        assert len(row_access_policy.EXAMPLES) > 0

    def test_examples_contain_fqcn(self):
        assert "stevefulme1.snowflake" in row_access_policy.EXAMPLES

    def test_return_exists(self):
        assert hasattr(row_access_policy, "RETURN")
        assert len(row_access_policy.RETURN) > 0
...
class TestCreate:
    ...
    def test_create_executes_sql(self, mock_cursor):
        ...
        mock_cursor.execute("SHOW ROW_ACCESS_POLICYS LIKE 'test'")
        mock_cursor.execute.assert_called()

    def test_create_new_resource(self, mock_cursor):
        mock_cursor.fetchone.return_value = None
        result = {"changed": True, "row_access_policy": {"name": "test"}}
        assert result["changed"] is True

    def test_create_existing_no_change(self, mock_cursor):
        mock_cursor.fetchone.return_value = ("test",)
        result = {"changed": False, "row_access_policy": {"name": "test"}}
...
class TestDelete:
    ...
    def test_delete_existing(self, mock_cursor):
        mock_cursor.fetchone.return_value = ("test",)
        mock_cursor.execute("DROP ROW_ACCESS_POLICY test")
        result = {"changed": True}
        assert result["changed"] is True
...
class TestErrorHandling:
    ...
    def test_connection_error(self, mock_cursor):
        ...
        with pytest.raises(Exception, match="Connection refused"):
            mock_cursor.execute("SHOW ROW_ACCESS_POLICYS")
    ...
    def test_permission_denied(self, mock_cursor):
        ...
        with pytest.raises(Exception, match="Insufficient privileges"):
            mock_cursor.execute("CREATE ROW_ACCESS_POLICY test")
